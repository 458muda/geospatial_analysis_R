...
bkt <- 'activemapper'
tiles_nicfi_path <- 'ecaas_2021/geoms/tiles_nicfi.geojson'
ecaas_tiles_path <- 'ecaas_2021/geoms/ecaas_tiles.geojson'

s2l2_path <- 'imagery/sentinel2/level2/'
#s2_footprints_path <- 'ecaas_2021/catalogs/s2_footprints_nghana_202102_202201.geojson'
s2_footprints_path <- 'ecaas_2021/catalogs/s2_footprints_ecaas_2021.geojson'
cat_prefix <- "ecaas_2021/catalogs/"

# read nicfi tiles
tiles_nicfi <- s3read_using(st_read, bucket = bkt, object = tiles_nicfi_path)
tiles_ecaas <- s3read_using(st_read, bucket = bkt, object = ecaas_tiles_path)
...
s2_geocat_plot <- do.call(rbind, lapply(s2_tiles_nghana_aws[[1]], function(x) {
  s2_geocat_sf %>% filter(grepl(x, title)) %>% .[5,]
})
) %>% st_as_sf()

s2_geocat_plot <- s2_geocat_plot %>% filter(grepl("30PWT", title))

one_tile <- tiles_ecaas %>% filter(tile == 591429)
...
ggplot() +
  ...
  geom_sf(data = one_tile$geometry)
